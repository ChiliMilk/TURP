...
public class ModelBakeSmoothNormalTool : AssetPostprocessor
{
    ...
    public struct BakeNormalJob : IJobParallelFor
    {
        ...
        [WriteOnly] public NativeArray<Vector2> uv3;

        public BakeNormalJob(NativeArray<Vector3> vertrx, NativeArray<Vector3> normals, NativeArray<Vector4> tangents, NativeArray<UnsafeHashMap<Vector3, Vector3>> result, NativeArray<Vector2> uv3)
        {
            this.vertrx = vertrx;
            this.normals = normals;
    ...
    {
        if (!gameObject.name.Contains("_SN") || gameObject.name.Contains("@@@"))
            return;
        ...
        else
        {
            ...
            Dictionary<string, Mesh> originalMesh = GetMesh(gameObject), smoothedMesh = GetMesh(go);
    ...
    void ComputeSmoothedNormalByJob(Mesh smoothedMesh, Mesh originalMesh, int maxOverlapvertices = 20)
    {
        ...
        var uv3 = new NativeArray<Vector2>(ovc, Allocator.Persistent);
        ...
        BakeNormalJob normalBakeJob = new BakeNormalJob(vertrxO, normalsO, tangents, result, uv3);

        normalBakeJob.Schedule(ovc, 8, collectNormalJob.Schedule(svc, 100)).Complete();

        var _uv3 = new Vector2[ovc];
        uv3.CopyTo(_uv3);
        originalMesh.uv3 = _uv3;
