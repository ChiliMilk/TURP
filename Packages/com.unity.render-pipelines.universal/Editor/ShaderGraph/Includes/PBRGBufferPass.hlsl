...
ToonData InitializeToonData(SurfaceDescription surfaceDescription)
{
    ToonData outToonData = (ToonData)0;
    outToonData.toonDiffuseRampV = surfaceDescription.ToonDiffuseRampV;
    outToonData.toonSpecularFeather = surfaceDescription.ToonSpecularFeather;
    outToonData.toonPointShadowPosition = 0.0;
...
FragmentOutput frag(PackedVaryings packedInput)
{
    ...
    ToonData toonData = InitializeToonData(surfaceDescription);

    return BRDFDataToGbufferToon(brdfData, inputData, toonData, surfaceDescription.Smoothness, surfaceDescription.Emission + color, surfaceDescription.Occlusion);
}
