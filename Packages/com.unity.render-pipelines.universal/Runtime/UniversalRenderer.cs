...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderer : ScriptableRenderer
    {
        ...
        //ToonShading
        DrawObjectsPass m_ToonOutlinePass;
        ToonDataPass m_ToonDataPass;
        ...
        //ToonShading
        RTHandle m_ToonDepthTexture;
        RTHandle m_ToonDataTexture;
        ...
        public UniversalRenderer(UniversalRendererData data) : base(data)
        {
            ...
            //ToonShading
            m_ToonDataPass = new ToonDataPass(RenderPassEvent.BeforeRenderingPrePasses, RenderQueueRange.all, data.opaqueLayerMask | data.transparentLayerMask);
            ...
            //ToonShading
            ShaderTagId[] toonOutlineShaderTagIds = new ShaderTagId[]
                {
                    new ShaderTagId("ToonOutline"),
                };
            m_ToonOutlinePass = new DrawObjectsPass("Toon Outline", toonOutlineShaderTagIds, true, RenderPassEvent.AfterRenderingTransparents, RenderQueueRange.all, data.opaqueLayerMask, m_DefaultStencilState, stencilData.stencilReference);
        ...
        internal override void ReleaseRenderTargets()
        {
            ...
            m_ToonDepthTexture?.Release();
            m_ToonDataTexture?.Release();
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            //ToonShading
            {
                var toonDepthDescriptor = cameraTargetDescriptor;
                toonDepthDescriptor.graphicsFormat = GraphicsFormat.None;
                toonDepthDescriptor.depthStencilFormat = GraphicsFormat.D16_UNorm;
                toonDepthDescriptor.depthBufferBits = 16;
                toonDepthDescriptor.msaaSamples = 1;
                RenderingUtils.ReAllocateIfNeeded(ref m_ToonDepthTexture, toonDepthDescriptor, FilterMode.Point, wrapMode: TextureWrapMode.Clamp, name: "_ToonDepthTexture");

                var toonDataDescriptor = cameraTargetDescriptor;
                toonDataDescriptor.graphicsFormat = GraphicsFormat.R8_UNorm;
                toonDataDescriptor.depthStencilFormat = GraphicsFormat.None;
                toonDataDescriptor.depthBufferBits = 0;
                toonDataDescriptor.msaaSamples = 1;
                RenderingUtils.ReAllocateIfNeeded(ref m_ToonDataTexture, toonDataDescriptor, FilterMode.Point, wrapMode: TextureWrapMode.Clamp, name: "_ToonDataTexture");

                cmd.SetGlobalTexture(m_ToonDepthTexture.name, m_ToonDepthTexture.nameID);
                cmd.SetGlobalTexture(m_ToonDataTexture.name, m_ToonDataTexture.nameID);
                context.ExecuteCommandBuffer(cmd);
                cmd.Clear();
            }
            ...
            //ToonShading
            m_ToonDataPass.Setup(m_ToonDataTexture, m_ToonDepthTexture);
            EnqueuePass(m_ToonDataPass);
