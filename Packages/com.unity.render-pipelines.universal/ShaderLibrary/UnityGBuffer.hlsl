...
//ToonShading
#define kMaterialFlagSDFShadow 2
#define kMaterialFlagDepthShadow 4
#define kMaterialFlagInLighting   8 
...
FragmentOutput SurfaceDataToGbufferToon(SurfaceData surfaceData, InputData inputData, ToonData toonData, half3 globalIllumination, int lightingMode)
{
    half3 packedNormalWS = PackNormal(inputData.normalWS);

    uint materialFlags = 0;
    materialFlags |= toonData.toonSDFShadowEnable > 0 ? kMaterialFlagSDFShadow : 0;
    materialFlags |= toonData.toonDepthShadowEnable > 0 ? kMaterialFlagDepthShadow : 0;
#ifdef _RECEIVE_SHADOWS_OFF
    materialFlags |= kMaterialFlagReceiveShadowsOff;
    materialFlags |= toonData.toonInLighting ? kMaterialFlagInLighting : 0;
#endif

    FragmentOutput output;
    output.GBuffer0 = half4(surfaceData.albedo.rgb, PackMaterialFlags(materialFlags));   // albedo          albedo          albedo          materialFlags   (sRGB rendertarget)
    output.GBuffer1 = half4(surfaceData.specular.r, toonData.toonDiffuseRampV, toonData.toonDiffuseRampOffset, toonData.toonStaticShadow);
...
FragmentOutput BRDFDataToGbufferToon(BRDFData brdfData, InputData inputData, ToonData toonData, half smoothness, half3 globalIllumination, half occlusion = 1.0)
{
    half3 packedNormalWS = PackNormal(inputData.normalWS);
    uint materialFlags = 0;
    materialFlags |= toonData.toonDepthShadowEnable > 0 ? kMaterialFlagDepthShadow : 0;
    ...
    packedSpecular.r = brdfData.reflectivity;
    packedSpecular.g = toonData.toonDiffuseRampV;
    packedSpecular.b = toonData.toonSpecularFeather;
...
ToonData ToonDataFromGBufferLit(half4 gbuffer1, bool DepthShadowEnable)
{
    ToonData toonData = (ToonData)0;
    toonData.toonDiffuseRampV = gbuffer1.g;
    toonData.toonSpecularFeather = gbuffer1.b;
    toonData.toonDepthShadowEnable = DepthShadowEnable ? 1.0 : 0.0;

    return toonData;
}

ToonData ToonDataFromGBufferSimpleLit(half4 gbuffer1, bool SDFShadowEnable, bool DepthShadowEnable)
{
    ToonData toonData = (ToonData)0;

    toonData.toonDiffuseRampV = gbuffer1.g;
    toonData.toonDiffuseRampOffset = gbuffer1.b;
    toonData.toonStaticShadow = gbuffer1.a;
    toonData.toonSDFShadowEnable = SDFShadowEnable ? 1.0 : 0.0;
    toonData.toonDepthShadowEnable = DepthShadowEnable ? 1.0 : 0.0;
...
BRDFData BRDFDataFromGbufferToon(half4 gbuffer0, half4 gbuffer1, half4 gbuffer2)
{
    half3 albedo = gbuffer0.rgb;
    half3 specular = gbuffer1.r;
    uint materialFlags = UnpackMaterialFlags(gbuffer0.a);
    half smoothness = gbuffer2.a;
