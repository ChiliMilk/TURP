...
FragmentOutput LitGBufferPassFragment(Varyings input)
{
    ...
    Light mainLight = GetMainLightNoShadow(inputData.positionWS);
    MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI, inputData.shadowMask);
    half3 color = GlobalIllumination(brdfData, inputData.bakedGI, surfaceData.occlusion, inputData.positionWS, inputData.normalWS, inputData.viewDirectionWS);

    //ToonShading
    ToonData toonData = InitializeToonData();
#ifdef _RECEIVE_SHADOWS_OFF
    toonData.toonInLighting = toonData.toonPointShadowEnable? MainLightRealtimeShadowPoint(toonData.toonPointShadowPosition) : 1.0;
#endif
    half3 matCap = SampleToonMatCap(inputData.normalWS);
    return BRDFDataToGbufferToon(brdfData, inputData, toonData, surfaceData.smoothness, surfaceData.emission + color + matCap, surfaceData.occlusion);
}
