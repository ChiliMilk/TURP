Shader "Hidden/Universal Render Pipeline/StencilDeferred"
{
    ...
    Light GetStencilLight(float3 posWS, float2 screen_uv, half4 shadowMask, uint materialFlags)
    {
        ...
        bool materialToonInLighting = (materialFlags & kMaterialFlagInLighting) != 0;
        uint lightLayerMask =_LightLayerMask;
                ...
                {
                    unityLight.shadowAttenuation = materialToonInLighting ? 1.0 : 0.0;
                }
    ...
    half4 DeferredShading(Varyings input) : SV_Target
    {
        ...
        bool DepthShadowEnable = (materialFlags & kMaterialFlagDepthShadow);
        
        #if defined(_DEFERRED_MAIN_LIGHT)
            //Toon Rim
            float linearEyeDepth = LinearEyeDepth(d, _ZBufferParams);
            float toonDepth = SAMPLE_TEXTURE2D_X_LOD(_ToonDepthTexture, sampler_ToonDepthTexture_Point_Clamp, screen_uv, 0).x;
            float linearToonEyeDepth = LinearEyeDepth(toonDepth, _ZBufferParams);

            float2 offsetUVRim = normalize(TransformWorldToViewDir(unityLight.direction).xy) * _ScreenSize.zw * lerp(_ToonRimWidth, 0.0, saturate(linearEyeDepth / _ToonRimMaxDistance));
            float toonDepthRim = SAMPLE_TEXTURE2D_X_LOD(_ToonDepthTexture, sampler_ToonDepthTexture_Point_Clamp, screen_uv + offsetUVRim, 0).x;
            float linearRimEyeDepth = LinearEyeDepth(toonDepthRim, _ZBufferParams);
            if ((linearRimEyeDepth - linearToonEyeDepth) > _ToonRimThreshold && (linearToonEyeDepth - linearEyeDepth < 0.01))
            {
                unityLight.color *= 1.0 + _ToonRimIntensity;
            }

            [branch] if (DepthShadowEnable)
            {
                //Toon DepthShadow
                float2 offsetUVShadow = normalize(TransformWorldToViewDir(unityLight.direction).xy) * _ScreenSize.zw * lerp(_ToonDepthShadowOffset, 0.0, saturate(linearEyeDepth / _ToonDepthShadowMaxDistance));
                float toonDepthShadow = SAMPLE_TEXTURE2D_X_LOD(_ToonDepthTexture, sampler_ToonDepthTexture_Point_Clamp, screen_uv + offsetUVShadow, 0).x;
                float lineaDepthShadowrEyeDepth = LinearEyeDepth(toonDepthShadow, _ZBufferParams);

                int toonID = (int)(SAMPLE_TEXTURE2D_X_LOD(_ToonDataTexture, sampler_ToonDataTexture_Point_Clamp, screen_uv, 0).x * 255.0);
                int toonIDOffset = (int)(SAMPLE_TEXTURE2D_X_LOD(_ToonDataTexture, sampler_ToonDataTexture_Point_Clamp, screen_uv + offsetUVShadow, 0).x * 255.0);
            ...
            #endif
            BRDFData brdfData = BRDFDataFromGbufferToon(gbuffer0, gbuffer1, gbuffer2);
            
            //ToonShading
            ToonData toonData = ToonDataFromGBufferLit(gbuffer1, DepthShadowEnable);
            color = LightingToonLit(brdfData, toonData, unityLight, inputData.normalWS, inputData.viewDirectionWS, DepthShadow, materialSpecularHighlightsOff);
        #elif defined(_SIMPLELIT)
            SurfaceData surfaceData = SurfaceDataFromGbuffer(gbuffer0, gbuffer1, gbuffer2, kLightingSimpleLit);
            bool SDFShadowEnable = (materialFlags & kMaterialFlagSDFShadow);
            ToonData toonData = ToonDataFromGBufferSimpleLit(gbuffer1, SDFShadowEnable, DepthShadowEnable);
            
            color = LightingToonSimpleLit(surfaceData, toonData, unityLight, inputData.normalWS, inputData.viewDirectionWS, DepthShadow);
        #endif
